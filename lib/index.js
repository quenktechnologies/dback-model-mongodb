...
exports.aggregate = exports.count = exports.removeAll = exports.remove = exports.get = exports.unsafeUpdate = exports.updateAll = exports.update = exports.search = exports.createAll = exports.create = exports.BaseModel = void 0;
...
class BaseModel {
    ...
    create(data) {
        return (0, exports.create)(this, data);
    }
    createAll(data) {
        return (0, exports.createAll)(this, data);
    }
    search(filter, opts) {
        return (0, exports.search)(this, filter, opts, this.refs);
    }
    update(id, changes, qry, opts) {
        return (0, exports.update)(this, id, changes, qry, opts);
    }
    updateAll(qry, changes, opts) {
        return (0, exports.updateAll)(this, qry, changes, opts);
    }
    unsafeUpdate(qry, spec, opts) {
        return (0, exports.unsafeUpdate)(this, qry, spec, opts);
    }
    get(id, qry, opts) {
        return (0, exports.get)(this, id, qry, opts, this.refs);
    }
    remove(id, qry, opts) {
        return (0, exports.remove)(this, id, qry, opts);
    }
    removeAll(qry, opts) {
        return (0, exports.removeAll)(this, qry, opts);
    }
    count(qry) {
        return (0, exports.count)(this, qry);
    }
    aggregate(pipeline, opts) {
        return (0, exports.aggregate)(this, pipeline, opts);
    }
}
...
const getIdQry = (model, id, qry) => {
    ...
    return (0, record_1.empty)(qry) ? idQry : { $and: [idQry, qry] };
};
/**
 * create a new document using a Model.
 */
const create = (model, data) => (0, future_1.doFuture)(function* () {
    ...
    return mDoc.isJust() ?
        (0, future_1.pure)(mDoc.get()[model.id]) :
        (0, future_1.raise)(new Error('create(): Could not retrieve ' +
            'target document!'));
});
 ...
 */
const createAll = (model, data) => (0, future_1.doFuture)(function* () {
    let result = yield noniMongo.insertMany(model.collection, data);
    let qry = { _id: { $in: (0, record_1.mapTo)(result.insertedIds, id => id) } };
    let opts = { projection: { [model.id]: 1 } };
    let results = yield noniMongo.find(model.collection, qry, opts);
    if (results.length !== data.length)
        return (0, future_1.raise)(new Error('createBulk(): ' +
            'Some documents were not created!'));
    return (0, future_1.pure)(results.map((r) => r[model.id]));
});
exports.createAll = createAll;
/**
 * search the Model's collection for documents matching the query.
 */
const search = (model, qry, opts = {}, refs = []) => (0, future_1.doFuture)(function* () {
    let actualOpts = (0, record_1.rmerge)({ projection: { _id: false } }, opts);
    ...
    return (0, future_1.pure)(results);
});
 ...
 * @returns - The number of documents that matched the query.
 */
const unsafeUpdate = (model, qry = {}, spec, opts = {}) => {
    return noniMongo.updateMany(model.collection, qry, spec, opts)
        .map(r => r.matchedCount);
};
 ...
 * Use sparingly.
 */
const get = (model, id, qry = {}, opts = {}, refs = []) => (0, future_1.doFuture)(function* () {
    let actualQry = getIdQry(model, id, qry);
    let mHit = yield noniMongo.findOne(model.collection, actualQry, opts);
    if (mHit.isNothing())
        return (0, future_1.pure)(mHit);
    for (let i = 0; i < refs.length; i++) {
        let [colname, lkey, fkey, fields] = refs[i];
        let col = model.database.collection(colname);
